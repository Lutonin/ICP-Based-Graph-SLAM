...
def compute_covariance(laser_ref, laser_sens, t, theta, angles):
    ...
    for i in range(laser_ref.shape[0]-1):
        ...
        p_j1 = laser_ref[i]
        p_j2 = laser_ref[i+1]
        v1 = np.dot(rotation_matrix(theta + np.pi/2),p_i)
        v2 = np.dot(rotation_matrix(theta), p_i) + t - p_j1
        v3 = vers(theta + angles[i])
        v4 = vers(theta + angles[i] + np.pi/2)

        C_k = compute_C_k(p_j1, p_j2)
        d2J_dt2_k = 2 * C_k
        d2J_dt_dtheta_k = 2 * np.dot(C_k, v1)

        v_new = np.dot(rotation_matrix(theta+np.pi), p_i)

        d2J_dtheta2_k = 2 * (np.dot(C_k , np.dot(v_new , v2.T)) + np.dot(C_k , np.dot(v1 , v1.T)))

        d2J_dt2 += d2J_dt2_k
        d2J_dt_dtheta += d2J_dt_dtheta_k[:, np.newaxis]
        d2J_dtheta2 += d2J_dtheta2_k
        d2Jk_dtdrho_i = 2 * np.dot(C_k , v3)
        d2Jk_dtheta_drho_i = 2 * (np.dot(C_k, np.dot(v4 , v2.T)) + np.dot(C_k , np.dot(v1, v3.T)))
        ...
        d2Jk_dt_drho_j1 = (-2 * np.dot(C_k , v_j1)) + (2 * np.dot(dC_drho_j1 , v2))
        d2Jk_dtheta_drho_j1 = (-2 * np.dot(C_k ,  np.dot(v1 ,v_j1.T))) + (np.dot(dC_drho_j1 , np.dot(v1 , v2.T)))
        d2J_dxdy1[:, i] += np.hstack([d2Jk_dt_drho_j1, d2Jk_dtheta_drho_j1])
        d2Jk_dt_drho_j2 = 2 * np.dot(dC_drho_j2 , v2)
        d2Jk_dtheta_drho_j2 =  2 * np.dot(dC_drho_j2 , np.dot(v1 , v2.T))
    ...
    edx_dy1 = np.dot(-np.linalg.inv(d2J_dx2) , d2J_dxdy1)
    edx_dy2 = np.dot(-np.linalg.inv(d2J_dx2) , d2J_dxdy2)

    ecov0_x = np.dot(edx_dy1 , edx_dy1.T) + np.dot(edx_dy2 , edx_dy2.T)
...
def icp(A, B, init_pose=None, max_iterations=20*4, tolerance=0.001/100):
    '''
    ...
    for i in range(max_iterations):
        ...
        indices = indices[distances < 1]
	
        # compute the transformation between
        # the current source and nearest destination points
        T,_,_ = best_fit_transform(filtered_src[:m, distances < 1].T, dst[:m,indices].T)
    ...
    return T, distances, i, cov, mean_error
